    ...
    public class IceBoss : ModNPC
    {
            ...
            switch (npc.ai[0])
            {
                ...
                case 1: // Spawn random shit from off screen
                    ...
                    if (fourthlife == false)
                    {
                        ...
                        if (npc.ai[1] % 60 == 0)
                        {
                            if (Main.netMode != NetmodeID.MultiplayerClient)
                            {
                                ...
                                {
                                    npc.netUpdate = true;
                                    Projectile.NewProjectile(new Vector2(player.Center.X + Main.rand.Next(1200, 1500), npc.Center.Y + Main.rand.Next(-360, 360)), new Vector2(Main.rand.Next(-11, -6), 0), ProjectileID.FrostBlastHostile, npc.damage / (Main.expertMode ? 4 : 2), 0f, Main.myPlayer);
                                }
                                ...
                                if (npc.life <= npc.lifeMax * 0.5f)
                                {
                                    ...
                                    {
                                        npc.netUpdate = true;
                                        Projectile.NewProjectile(new Vector2(player.Center.X - Main.rand.Next(1200, 1500), npc.Center.Y + Main.rand.Next(-360, 360)), new Vector2(Main.rand.Next(6, 11), 0), ProjectileID.FrostBlastHostile, npc.damage / (Main.expertMode ? 4 : 2), 0f, Main.myPlayer);
                                    }
                                }
                            }
                        }
                    ...
                    else
                    {
                        ...
                        if (npc.ai[1] % /*30*/15 == 0)
                        {
                            Projectile.NewProjectileDirect(npc.Center, npc.DirectionTo(/*playercentersnapshot*/player.Center) * 7.5f, ProjectileID.FrostBlastHostile, npc.damage / (Main.expertMode ? 4 : 2), 0, Main.myPlayer, 0, npc.ai[3]);
                        }
                    }
                ...
                case 3: // Spin in a circle
                    ...
                    if (npc.ai[1] % 50 == 0)
                    {
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                        {
                            for (int i = 0; i < 2; i++)
                            {
                                Projectile.NewProjectile(npc.Center, Vector2.One.RotatedByRandom(Math.PI) * 6, ProjectileID.FrostBlastHostile, npc.damage / 4, 0f, Main.myPlayer);
                            }
                        }
                    }
        ...
        public override void NPCLoot()
        {
            ...
            else
            {
                ...
                else if (choice == 4) // Magus
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<ElementalPurge>());
                }
