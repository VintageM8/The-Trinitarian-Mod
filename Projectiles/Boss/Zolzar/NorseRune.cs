...
{
    public class NorseRune : ModProjectile
    {
        ...
        {
            DisplayName.SetDefault("Norse Rune Circle");
        }
                                    ...
                                    for (int k = 0; k < Main.maxPlayers; k++)
                                    {
                                        if (Main.player[k].active && !Main.player[k].dead)
                                        {
                                            ...
                                            if (distanceTo < distance)
                                            {
                                                if (Main.player[k].active && !Main.player[k].dead)
                                                {
                                                    Projectile.NewProjectile(projectile.Center, projectile.DirectionTo(Main.player[k].Center) * shootSpeed, ProjectileID.PhantasmalBolt, 4, 3f, Main.myPlayer, 0, 0);
                                                }
                                                distance = distanceTo;
                                            }
                                        }
                ...
                case 3: // shoot to the sides
                    {
                        ...
                        if (projectile.ai[1] % 45 == 0)
                        {
                            for (int i = 0; i < 5; i++)
                            {
                                if (Main.netMode != NetmodeID.MultiplayerClient)
                                {
                                    Projectile.NewProjectile(projectile.Center, new Vector2(Main.rand.Next(-5, -3) * directionalStore, Main.rand.Next(-3, 3) * directionalStore), ProjectileType<HomingThing>(), projectile.damage, 4, Main.myPlayer);
                                }
                            }
                        }
                    }
