...
namespace Trinitarian.Projectiles.Magus.Runes
{
    public class DarkRune : ModProjectile
    {
        ...
        {
            DisplayName.SetDefault("Dark Rune");
        }
            ...
            else
            {
                ...
                for (int i = 0; i < Main.maxNPCs; i++)
                {
                    ...
                    if (distance <= 400 && !Main.npc[i].friendly && projectile.ai[0] >= 90)
                    {
                        ...
                        {
                            Projectile.NewProjectile(projectile.Center, Vector2.One.RotatedByRandom(Math.PI) * 4, ProjectileType<DarkEnergy>(), projectile.damage, 3f, Main.myPlayer);
