...
{
    public class WaterStormBottom : ModProjectile
    {
        int delay = 0;
        float theta = 0;
        int num = 1;
        ...
        private void SpawnSandnadoLayers()
        {
            if (num < 5)
            {
                ...
                {
                    delay = 0;
                    Projectile.NewProjectile(new Vector2(projectile.Center.X, projectile.Center.Y - projectile.height * projectile.scale), new Vector2(0, 0), ModContent.ProjectileType<WaterStormUp>(), projectile.damage, 0, projectile.owner, projectile.whoAmI, num + 1);
                    num++;
                }
            }
        }
