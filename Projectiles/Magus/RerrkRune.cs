...
{
    public class RerrkRune : OrbitingProjectile
    {
        private float projectileRadius = 100;
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Rerrk Rune");
        }
            ...
            for (int i = 0; i < Main.maxPlayers; i++)
            {
                if (Main.player[i].active)
                {
                    ...
                    if (distance <= projectileRadius)
                    {
                        ...
                        {
                            Main.player[i].AddBuff(BuffID.Battle, 2800);
                            Main.player[i].AddBuff(BuffID.Rage, 2800);
                            Main.player[i].AddBuff(BuffID.Regeneration, 2800);
                        }
                    }
                }
            }
