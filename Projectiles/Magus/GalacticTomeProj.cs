...
{
    public class GalacticTomeProj : ModProjectile
    {
        public override void SetStaticDefaults()
        {
            base.DisplayName.SetDefault("Cosmic Power");
        ...
        public override void AI()
        {
            ...
            if (base.projectile.ai[0] == 0f && base.projectile.localAI[0] == 0f)
            {
                ...
                {
                    NPC nPC = Main.npc[j];
                    if (nPC.CanBeChasedBy(base.projectile) && (base.projectile.ai[0] == 0f || base.projectile.ai[0] == (float)j + 1f))
                    {
                        Vector2 vector2 = nPC.position + nPC.Size * value3;
                        float num5 = Vector2.Distance(vector2, vector);
                        if (num5 < num4 && Collision.CanHit(base.projectile.position, base.projectile.width, base.projectile.height, nPC.position, nPC.width, nPC.height))
                        {
                            num4 = num5;
                            vector = vector2;
                            num3 = j;
                ...
                if (num3 >= 0)
                {
                    base.projectile.ai[0] = (float)num3 + 1f;
