        ...
        public override void Hurt(bool pvp, bool quiet, double damage, int hitDirection, bool crit)
        {
            ...
            if (SteelSet)
            {
                ...
                if (Main.netMode != NetmodeID.MultiplayerClient && Main.myPlayer == Player.whoAmI)
                {
                    for (int i = 0; i < projectiles; i++)
                    {
                         Projectile.NewProjectile(Player.GetSource_OnHurt(null), Player.Center, new Vector2(7).RotatedBy(MathHelper.ToRadians((360 / projectiles) * i + i)), ModContent.ProjectileType<SteelPlate>(), 19, 2, Player.whoAmI);
                    }
                }
            }
